...
var ASM_CONSTS = {
  174349: ($0) => { Module['firstGLContextExt'] = GL.contexts[$0].GLctx.getExtension('WEBGL_lose_context'); },  
 174441: () => { Module['firstGLContextExt'].loseContext(); }
};
function get_memory_size() { return HEAP8.length; }
...
function After_Stream_To_Segment(key,no) { if (Module.onStreamToSegmentFinished) { Module.onStreamToSegmentFinished(key, no); } }
...
function On_Collision_Computed(view_key,count) { if (Module.onCollisionComputed) { Module.onCollisionComputed(view_key, count); } }
function After_Compute_Collision(no,count,col_keys,col_paths,col_poss,col_types) { if (Module.onComputeCollisionFinished) { let arr_list = []; for (let i = 0; i < count; i++) { let item = { keyOne: Module.HEAP32[col_keys / 4 + i * 2], keyTwo : Module.HEAP32[col_keys / 4 + i * 2 + 1], pathOne : UTF8ToString(Module.HEAP32[col_paths / 4 + i * 2]), pathTwo : UTF8ToString(Module.HEAP32[col_paths / 4 + i * 2 + 1]), position : [Module.HEAPF32[col_poss / 4 + i * 3], Module.HEAPF32[col_poss / 4 + i * 3 + 1], Module.HEAPF32[col_poss / 4 + i * 3 + 2]], type : Module.HEAP32[col_types / 4 + i] }; arr_list.push(item); } Module.onComputeCollisionFinished(no, arr_list); } }
...
var ___start_em_js = Module['___start_em_js'] = 171580;
var ___stop_em_js = Module['___stop_em_js'] = 174349;
